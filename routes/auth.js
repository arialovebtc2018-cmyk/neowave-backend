...
// 4. فراموشی رمز عبور (Reset Password)
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (!user) return res.status(404).json("کاربری با این شماره یافت نشد.");
        ...
        res.status(200).json("رمز عبور با موفقیت تغییر کرد. حالا می‌توانید وارد شوید.");
    } catch (err) {
        res.status(500).json(err);
    }
});
